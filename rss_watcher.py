    ...
    def data(self):
        ...
        if now > self._feed_updated_at + self.update_timeout:
            ...
            if not self._feed.feed:
                raise ValueError(f"Cannot initialising feed {self.feed_url}. Check the connection.")
            self._feed_updated_at = datetime.fromtimestamp(mktime(self._feed.feed.updated_parsed)).replace(tzinfo=timezone.utc)
